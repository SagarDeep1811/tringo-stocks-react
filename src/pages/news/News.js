...
function News() {
  ...
  useEffect(() => {
    axios
      .get(`${BASE_URL}?category=general&token=${TOKEN}`)
      .then((response) => {
  ...
  console.log("rendering the news component...");
  if (data != null) {
    dataToBeProcessed = data;
    console.log("data : ", dataToBeProcessed);
    console.log("typeof data : ", typeof dataToBeProcessed);
    console.log("data[0] : ", dataToBeProcessed[0]);
  }
  else {
    dataToBeProcessed = dummyData;
    console.log("data : ", dataToBeProcessed);
    console.log("typeof data : ", typeof dataToBeProcessed);
    console.log("data[0] : ", dataToBeProcessed[0]);
  }
  
  const numberOfNewsPalettes = [...Object.keys(dataToBeProcessed)].length;
  const propertyOfEachNewsPalettes = [...Object.values(dataToBeProcessed)];
  ...
  const next = (e) => {
    ...
    if (indicesArray[0] < numberOfNewsPalettes - 2) {
      setIndicesArray((indicesArray) =>
        [...indicesArray].map((index) => (index = index + 1))
      );
    }
    if (indicesArray[0] === numberOfNewsPalettes - 2) {
      setIndicesArray([0, 1]);
      ...
      <NewsContainer>
        <TimestampAndCategory>
          ...
          <div style={{ height: "auto" }}>
            <h3>{dataToBeProcessed[startingIndex].headline}</h3>
            ...
            >
              {dataToBeProcessed[startingIndex].summary}
            </p>
            ...
            <img
              ...
              src={dataToBeProcessed[startingIndex].image}
            />

            <a href={dataToBeProcessed[startingIndex].url}>
            ...
            <p
              style={{
                color: "white",
                textAlign: "center",
                backgroundColor: "rgb(88, 81, 81)",
              }}
            >
              Source : {dataToBeProcessed[startingIndex].source}
            </p>
            <p
              style={{
                color: "black",
                backgroundColor: "rgb(143, 134, 229)",
                textAlign: "center",
              }}
            >
              {dataToBeProcessed[startingIndex].category}
            </p>
          </div>
          {
